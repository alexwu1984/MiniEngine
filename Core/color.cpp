...
namespace core
{
	...
	*/
	...
	FLinearColor::FLinearColor(const math::Vector3& Vector) :
		R(Vector.x),
		G(Vector.y),
		B(Vector.z),
		A(1.0f)
	...
	FLinearColor::FLinearColor(const math::Vector4& Vector) :
		R(Vector.x),
		G(Vector.y),
		B(Vector.z),
		A(Vector.w)
	...
	FColor FLinearColor::ToRGBE() const
	{
		...
		else
		{
			...
			Color.R = (uint8_t)math::Clamp(math::TruncToInt(R * Scale), 0, 255);
			Color.G = (uint8_t)math::Clamp(math::TruncToInt(G * Scale), 0, 255);
			Color.B = (uint8_t)math::Clamp(math::TruncToInt(B * Scale), 0, 255);
			Color.A = (uint8_t)(math::Clamp(Exponent, -128, 127) + 128);
		}
	...
	FColor FLinearColor::ToFColor(const bool bSRGB) const
	{
		...
		Result.A = (uint8_t)math::FloorToInt(FloatA * 255.999f);
		Result.R = (uint8_t)math::FloorToInt(FloatR * 255.999f);
		Result.G = (uint8_t)math::FloorToInt(FloatG * 255.999f);
		Result.B = (uint8_t)math::FloorToInt(FloatB * 255.999f);
	...
	FLinearColor FLinearColor::Desaturate(float Desaturation) const
	{
		...
		return math::Lerp(*this, FLinearColor(Lum, Lum, Lum, 0), Desaturation);
	}
	...
	{
		const uint8_t Hue = (uint8_t)(math::FRand() * 255.f);
	...
	{
		return FColor(
			(uint8_t)math::Clamp<int32_t>(math::TruncToInt(R * 255.f), 0, 255),
			(uint8_t)math::Clamp<int32_t>(math::TruncToInt(G * 255.f), 0, 255),
			(uint8_t)math::Clamp<int32_t>(math::TruncToInt(B * 255.f), 0, 255),
			(uint8_t)math::Clamp<int32_t>(math::TruncToInt(A * 255.f), 0, 255)
		);
	}
	...
	{
		return FColor(
			(uint8_t)math::Clamp<int32_t>(math::RoundToInt(R * 255.f), 0, 255),
			(uint8_t)math::Clamp<int32_t>(math::RoundToInt(G * 255.f), 0, 255),
			(uint8_t)math::Clamp<int32_t>(math::RoundToInt(B * 255.f), 0, 255),
			(uint8_t)math::Clamp<int32_t>(math::RoundToInt(A * 255.f), 0, 255)
		);
	}
	...
	FLinearColor FColor::FromRGBE() const
	{
		...
		{
			const float Scale = (float)std::ldexp(1 / 255.0f, A - 128);
			return FLinearColor(R * Scale, G * Scale, B * Scale, 1.0f);
		}
	}

	FLinearColor FColor::MakeFromHSV8(uint8_t H, uint8_t S, uint8_t V)
	{
		// want a given H value of 255 to map to just below 360 degrees
		const FLinearColor HSVColor((float)H * (360.0f / 256.0f), (float)S / 255.0f, (float)V / 255.0f);
		return HSVColor.HSVToLinearRGB();
	...
	{
		const float RedSclr = math::Clamp<float>((1.0f - Scalar) / 0.5f, 0.f, 1.f);
		const float GreenSclr = math::Clamp<float>((Scalar / 0.5f), 0.f, 1.f);
		const uint8_t R = (uint8_t)math::TruncToInt(255 * RedSclr);
		const uint8_t G = (uint8_t)math::TruncToInt(255 * GreenSclr);
		const uint8_t B = 0;
		return FColor(R, G, B);
	}
