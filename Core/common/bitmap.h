...
public:
	...
	*/
	static std::shared_ptr<Bitmap> bitmapFromFile(const std::wstring& filePath);

	static Bitmap bitmapFromMemory(const uint8_t *buffer, int len);
	...
	*/
	...
	unsigned char* getLinePixel(unsigned int row) const;
	...
	*/
	...
	Bitmap scale(unsigned int width, unsigned int height);
	Bitmap linear_scale(unsigned int width, unsigned int height);

	void SaveBitmap(const wchar_t* fileName);
