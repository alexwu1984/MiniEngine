...
static FORCEINLINE uint16_t BYTESWAP_ORDER16(uint16_t Val)
{
...
static FORCEINLINE int16_t BYTESWAP_ORDER16(int16_t Val)
{
	return static_cast<int16_t>(BYTESWAP_ORDER16(static_cast<uint16_t>(Val)));
}

static FORCEINLINE uint32_t BYTESWAP_ORDER32(uint32_t Val)
{
...
static FORCEINLINE int32_t BYTESWAP_ORDER32(int32_t val)
{
	return static_cast<int32_t>(BYTESWAP_ORDER32(static_cast<uint32_t>(val)));
}

static FORCEINLINE uint64_t BYTESWAP_ORDER64(uint64_t Value)
{
...
static FORCEINLINE int64_t BYTESWAP_ORDER64(int64_t Value)
{
	return static_cast<int64_t>(BYTESWAP_ORDER64(static_cast<uint64_t>(Value)));
}

static FORCEINLINE float BYTESWAP_ORDERF(float val)
{
	uint32_t uval = BYTESWAP_ORDER32(*reinterpret_cast<const uint32_t*>(&val));
	return *reinterpret_cast<const float*>(&uval);
}

static FORCEINLINE double BYTESWAP_ORDERD(double val)
{
	uint64_t uval = BYTESWAP_ORDER64(*reinterpret_cast<const uint64_t*>(&val));
...
static FORCEINLINE void BYTESWAP_ORDER_TCHARARRAY(TCHAR* str)
{
	static_assert(sizeof(TCHAR) == sizeof(uint16_t), "Assuming TCHAR is 2 bytes wide.");
	for (TCHAR* c = str; *c; ++c)
	{
		*c = BYTESWAP_ORDER16(static_cast<uint16_t>(*c));
	}
}
...
template<> inline int16_t ByteSwap(int16_t Value)   { return BYTESWAP_ORDER16(Value); }
template<> inline uint16_t ByteSwap(uint16_t Value) { return BYTESWAP_ORDER16(Value); }
template<> inline int32_t ByteSwap(int32_t Value)   { return BYTESWAP_ORDER32(Value); }
template<> inline uint32_t ByteSwap(uint32_t Value) { return BYTESWAP_ORDER32(Value); }
template<> inline int64_t ByteSwap(int64_t Value)   { return BYTESWAP_ORDER64(Value); }
template<> inline uint64_t ByteSwap(uint64_t Value) { return BYTESWAP_ORDER64(Value); }
...
template<> inline char16_t ByteSwap(char16_t Value) { return static_cast<char16_t>(BYTESWAP_ORDER16(static_cast<uint16_t>(Value))); }
