...
namespace RenderCore
{
	namespace D3D12ShaderUtils
	{
		...
		struct FRootSignatureCreator
		{
			...
			virtual void AddTable(EShaderFrequency Stage, EType Type, int32_t NumDescriptors/*, bool bAppend*/) = 0;
		...
		struct FBinaryRootSignatureCreator : public FRootSignatureCreator
		{
			...
			std::map<uint32_t, uint32_t> ParameterToRangeMap;
			...
			virtual void AddTable(EShaderFrequency Stage, EType Type, int32_t NumDescriptors)
			{
				...
				ParameterToRangeMap.insert({ ParameterIndex, RangeIndex });
			...
			void Compile()
			{
				...
				{
					Parameters[Pair.first].DescriptorTable.pDescriptorRanges = &DescriptorRanges[Pair.second];
				}
			}
		};
	}
