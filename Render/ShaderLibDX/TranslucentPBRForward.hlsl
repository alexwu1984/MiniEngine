...
PS_OUTPUT_TRANSLUCENT_FWD MainPS_TranslucentForward(VS_OUTPUT_SCENE Input)
{
	PS_OUTPUT_TRANSLUCENT_FWD Out;
	Out.Velocity = float4(Calculate3DVelocity(Input.svCurrPosition, Input.svPrevPosition), 0.0);
	...
	Out.SceneColor = float4(color, alpha);
	return Out;
}
