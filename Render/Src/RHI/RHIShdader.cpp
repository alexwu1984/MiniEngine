...
namespace RenderCore
{
	...
	std::vector< VertexElementDesc> RHIVertexDeclare::CreateDeclare(const std::vector< VertexDeclareInput>& Inputs)
	{
		std::vector< VertexElementDesc> DescResult{};
		//Default create separate able layout
		uint32_t index = 0;
		for (const auto& Item: Inputs)
		{
			VertexElementDesc D3DElement{};
			switch (Item.InElementType)
			{
			...
			D3DElement.SemanticIndex = Item.InAttributeIndex;
			D3DElement.InputSlot = index++;
			//Item.bUseInstanceIndex ? D3D11_INPUT_PER_INSTANCE_DATA : D3D11_INPUT_PER_VERTEX_DATA;
			D3DElement.InputSlotClass = Item.bUseInstanceIndex ? 1 : 0;

			// This is a divisor to apply to the instance index used to read from this stream.
			D3DElement.InstanceDataStepRate = Item.bUseInstanceIndex ? 1 : 0;

			DescResult.emplace_back(D3DElement);
		}
